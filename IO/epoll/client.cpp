...
void do_read(int epollfd, int fd, int sockfd, char* buf){
    ...
    if(n_read == -1){
        ...
        if(fd == STDIN_FILENO){ //read from cin
            add_event(epollfd, sockfd, EPOLLOUT); //add write_event
        }
...
void do_write(int epollfd, int fd, int sockfd, char* buf){
    ...
    char temp[100];
    buf[strlen(buf) - 1] = '\0';
    snprintf(temp, sizeof(temp), "% s_%02d\n", buf, count++); //format buf to temp
    n_write = write(fd, temp, strlen(temp));
    if(n_write == -1){
        perror("write error");
        close(fd);
    }
    else{
        if(fd == STDOUT_FILENO){ //write to cout
